/*
• Prim's algorithm is used to find the minimum spanning tree (MST) of a connected, 
  undirected graph with weighted edges.
• It starts with an arbitrary vertex and grows the MST by adding the minimum weight 
  edge that connects the current MST to a vertex not yet included in the MST. 
  This process is repeated until all vertices are included in the MST.
• Prim's algorithm has a time complexity of O(E + V log V) with the help of efficient data structures 
  like a priority queue or a binary heap, where E is the number of edges and V is the number of vertices.
  It is efficient for dense graphs and can be faster than Kruskal's algorithm for sparse graphs.
*/
